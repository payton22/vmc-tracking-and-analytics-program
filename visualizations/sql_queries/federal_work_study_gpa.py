# Returns the SQL query for federal work study vs. gpa based on user-provided location, from date, and to date
def get_query(type, from_time, to_time, substr):
    return "SELECT demographics.federal_work_study, round(avg(gpa." + type + "), 2) " \
           "FROM " \
           ...
           "WHERE demographics.federal_work_study IS NOT NULL " \
           "GROUP BY demographics.federal_work_study;"