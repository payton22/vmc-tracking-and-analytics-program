def get_query(group_by, from_time, to_time, substr):
    ...
    if group_by == 'check_in_date' or group_by == 'location' or group_by == 'services':
        return "SELECT " + group_by + ", ROUND(COUNT(" + group_by + ") / (julianday('" + to_time\
        + "') - julianday('" + from_time + \
    ...
    # Else, a table join with demographics + visits is needed
        ...
        return "SELECT IFNULL(demographics." + group_by + ", ROUND(COUNT(demographics." + group_by + ") / (julianday('" + to_time \
               + "') - julianday('" + from_time + \
