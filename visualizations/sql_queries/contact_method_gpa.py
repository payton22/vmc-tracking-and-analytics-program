# Returns the SQL query for contact method vs. gpa based on user-provided location, from date, and to date
def get_query(type, from_time, to_time, substr):
    return "SELECT demographics.contact_method, round(avg(gpa." + type + "), 2) " \
           "FROM " \
           ...
           "WHERE demographics.contact_method IS NOT NULL " \
           "GROUP BY demographics.contact_method;"