# Returns the SQL query for marital status vs. gpa based on user-provided location, from date, and to date
def get_query(type, from_time, to_time, substr):
    return "SELECT demographics.marital_status, round(avg(gpa." + type + "), 2) " \
           "FROM " \
           ...
           "WHERE demographics.marital_status IS NOT NULL " \
           "GROUP BY demographics.marital_status;"