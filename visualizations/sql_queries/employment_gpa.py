# Returns the SQL query for employment vs. gpa based on user-provided location, from date, and to date
def get_query(type, from_time, to_time, substr):
    return "SELECT demographics.employment, round(avg(gpa." + type + "), 2) " \
           "FROM " \
           ...
           "WHERE demographics.employment IS NOT NULL " \
           "GROUP BY demographics.employment;"