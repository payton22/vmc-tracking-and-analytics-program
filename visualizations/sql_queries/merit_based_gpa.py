# Returns the SQL query for merit-based scholarships vs. gpa based on user-provided location, from date, and to date
def get_query(type, from_time, to_time, substr):
    return "SELECT demographics.merit_based, round(avg(gpa." + type + "), 2) " \
           "FROM " \
           ...
           "WHERE demographics.merit_based IS NOT NULL " \
           "GROUP BY demographics.merit_based;"