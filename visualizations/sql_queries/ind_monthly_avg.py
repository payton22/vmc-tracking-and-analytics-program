def get_query(group_by, from_time, to_time, substr):
    ...
    if group_by == 'total_usage_by_location':
        ...
        return "SELECT gpa." + group_by + ", round(count(gpa." + group_by + ")/(strftime('%m', '" + to_time + "') - strftime('%m', '" + from_time + "')), 2) FROM visits LEFT JOIN gpa ON visits.student_id = gpa.student_id WHERE (location = \'" + substr + "\') and check_in_date >= \'" + from_time + "\' AND check_in_date <= \'" + to_time + "\' and gpa." + group_by + " is not null GROUP BY gpa." + group_by + "; "
    else:
