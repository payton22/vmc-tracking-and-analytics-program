# Returns the SQL query for the monthly average visits by date based on user-provided location, from date, and to date
def get_query(group_by, from_time, to_time, substr):
    ...
    if group_by == 'total_usage_by_location':
        return "SELECT location, round(count(*)/(strftime('%m', '" + to_time + "') - " \
                "strftime('%m', '" + from_time + "')), 2) FROM visits  " \
                "WHERE (location = \'" + substr + "\') and check_in_date >= \'" + from_time + "\' " \
                "AND check_in_date <= \'" + to_time + "\' GROUP BY location;"
    # Query by date
    elif group_by == 'usage_by_date':
        return "SELECT check_in_date, round(count(*)/(strftime('%m', '" + to_time + "') - " \
                "strftime('%m', '" + from_time + "')), 2) FROM visits  " \
                "WHERE (location = \'" + substr + "\') and check_in_date >= \'" + from_time + "\' " \
    ...
    # No table joins - services comes from the visits table
        ...
        return "SELECT services, round(count(*)/(strftime('%m', '" + to_time + "') - " \
                "strftime('%m', '" + from_time + "')), 2) FROM visits  WHERE (location = \'" + substr + "\') " \
    ...
    # Table join with the GPA table
        ...
        return "SELECT gpa." + group_by + ", round(count(gpa." + group_by + ")/(strftime('%m', '" + to_time + "') - strftime('%m', '" + from_time + "')), 2) " \
                "FROM visits LEFT JOIN gpa ON visits.student_id = gpa.student_id " \
                "WHERE (location = \'" + substr + "\') and check_in_date >= \'" + from_time + \
               "\' AND check_in_date <= \'" + to_time + "\' and gpa." + group_by + " is not null " \
                "GROUP BY gpa." + group_by + "; "
    # Table join with the demographics table
    else:
        return "SELECT demographics." + group_by + ", round(count(demographics." + group_by + ")/(strftime('%m', '" + to_time + "') - " \
                "strftime('%m', '" + from_time + "')), 2) FROM visits " \
