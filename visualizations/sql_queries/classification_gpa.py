# Returns the SQL query for classification vs. gpa by date based on user-provided location, from date, and to date
def get_query(type, from_time, to_time, substr):
    return "SELECT demographics.classification, round(avg(gpa." + type + "), 2) " \
           "FROM " \
           ...
           "WHERE demographics.classification IS NOT NULL " \
           "GROUP BY demographics.classification;"